...
export const getEnterpriseAppAssignments = async (appId: string): Promise<AppRoleAssignment[]> => {
	return await graphGet<AppRoleAssignment[]>(`servicePrincipals(appId='${appId}')/appRoleAssignedTo?$select=id,appRoleId,principalDisplayName,principalId,principalType&$top=999`)
}

export const getGroupMembers = async (groupId: string): Promise<User[]> => {
	return await graphGet<User[]>(`groups/${groupId}/members`)
}
...
export class EntraClient implements IEntraClient {
	async getEnterpriseApplicationUsers(): Promise<User[]> {
		...
		*/
		...
		const appRoleAssignments: AppRoleAssignment[] = await getEnterpriseAppAssignments(FRONTEND_APP_REGISTRATION_ID)
		...
		for (const assignment of groupAssignments) {
			...
			const members = await getGroupMembers(assignment.principalId)
