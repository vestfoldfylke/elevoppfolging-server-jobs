...
export const generateMockFintSchoolsWithStudents = (config: GenerateMockFintSchoolsWithStudentsOptions): FintSchoolWithStudents[] => {
  ...
  // insert any predefined students
  for (const schoolStudents of config.predefinedSchoolStudents) {
    if (!schoolStudents.elevforhold) {
      logger.error("elevforhold is undefined on school with school number {SchoolNumber}, skipping", schoolStudents.skolenummer.identifikatorverdi)
      continue
    }

    const schoolWithStudents: FintSchoolWithStudents | undefined = schools.find(
      (school: FintSchoolWithStudents) => school.skole?.skolenummer?.identifikatorverdi === schoolStudents.skolenummer.identifikatorverdi
    )
    if (!schoolWithStudents?.skole) {
      logger.error(
        "Could not find school with school number {SchoolNumber} to add {PredefinedStudentCount} predefined students, skipping",
        schoolStudents.skolenummer.identifikatorverdi,
        schoolStudents.elevforhold.length
      )
      continue
    }

    if (!Array.isArray(schoolWithStudents.skole?.elevforhold)) {
      logger.error("Could not find elevforhold array on school with school number {SchoolNumber}", schoolStudents.skolenummer.identifikatorverdi)
      continue
    }

    for (let i: number = 0; i < schoolStudents.elevforhold.length; i++) {
      const elevforhold: FintElevforhold | null = schoolStudents.elevforhold[i]
      if (!elevforhold) {
        logger.error("elevforhold at index {ElevforholdIndex} is null on school with school number {SchoolNumber}, skipping", i, schoolStudents.skolenummer.identifikatorverdi)
        continue
      }

      schoolWithStudents.skole.elevforhold.push(elevforhold)
      logger.info(
        "Added predefined student with feidenavn {FeideName} to school with school number {SchoolNumber}",
        elevforhold.elev.feidenavn?.identifikatorverdi,
        schoolStudents.skolenummer.identifikatorverdi
      )
    }
  }
